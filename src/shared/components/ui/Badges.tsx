  ...
  if (tone === 'chip') {
    return (
      <View className="flex-row items-center gap-1.5 self-start rounded-full bg-gold/10 px-2.5 py-1.5">
        ...
        <Text className="font-heading text-[10px] text-gold">Vérifiée</Text>
      </View>
    );
  }
  return (
    <View className="flex-row items-center gap-1 self-start rounded-full border border-surface-border/95 bg-surface/[0.88] px-2.5 py-1.5">
      <BadgeCheck size={10} color={colors.gold.DEFAULT} strokeWidth={2.8} />
      <Text className="font-heading-semibold text-[10px] text-ink">Vérifiée</Text>
    </View>
  );
}
...
export function MatchBadge({ percent }: { percent: number }) {
  return (
    <View className="flex-row items-center gap-1.5 self-start rounded-full border border-surface-border/95 bg-surface/[0.88] px-3 py-1.5">
      <Heart size={11} color={colors.brand.DEFAULT} fill={colors.brand.DEFAULT} />
      <Text className="font-heading text-[11px] text-ink">{percent}% Match</Text>
    </View>
  );
}
